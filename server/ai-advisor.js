...
function buildPrompt(message, history, contextText) {
    var routingQuery = __spreadArray(__spreadArray([], history.filter(function (entry) { return entry.role === 'user'; }).slice(-2).map(function (entry) { return entry.content; }), true), [message], false).join(' ');
